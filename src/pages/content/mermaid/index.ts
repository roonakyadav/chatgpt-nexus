...
export const loadMermaid = async (): Promise<typeof mermaidInstance> => {
  ...
  try {
    ...
    console.error('[GPT-Nexus] Failed to load Mermaid library:', error);
    return null;
  }
};
...
const renderMermaid = async (
  ...
  try {
    ...
    codeBlock.dataset.mermaidProcessing = 'false';
    console.log('[GPT-Nexus] Mermaid diagram rendered:', uniqueId);
  } catch {
    codeBlock.dataset.mermaidProcessing = 'false';
...
export const startMermaid = () => {
  ...
  chrome.storage?.sync?.get({ gvMermaidEnabled: true }, (result) => {
    ...
    if (mermaidEnabled) {
      ...
      console.log('[GPT-Nexus] Mermaid rendering is disabled');
    }
  });
  ...
  chrome.storage?.onChanged?.addListener((changes, areaName) => {
    if (areaName === 'sync' && changes.gvMermaidEnabled) {
      ...
      if (mermaidEnabled) {
        initializeMermaid();
        console.log('[GPT-Nexus] Mermaid rendering enabled');
      } else {
        ...
        console.log('[GPT-Nexus] Mermaid rendering disabled');
      }
    }
  });
};
...
const initializeMermaid = async () => {
  ...
  console.log('[GPT-Nexus] Mermaid integration started');
};
