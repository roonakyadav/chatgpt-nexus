...
async function loadDictionaries(): Promise<Record<AppLanguage, Record<string, string>>> {
  try {
    const enRaw = await import(/* @vite-ignore */ '../../../locales/en/messages.json');
...
function getConversationTitleForExport(): string {
  // Strategy 1: Get from active conversation in GPT-Nexus Folder UI (most accurate)
  try {
...
async function executeExportSequence(
  ...
  if (state.attempt > 25) {
    console.warn('[GPT-Nexus] Export aborted: too many attempts.');
  ...
  // 1. Find Top Node
  if (state.attempt > 0) {
    console.log('[GPT-Nexus] Resuming export... waiting for content load.');
    const userSelectors = getUserSelectors();
  ...
  if (!topNode) {
    console.log('[GPT-Nexus] No top node found, proceeding to export directly.');
    sessionStorage.removeItem(SESSION_KEY_PENDING_EXPORT);
  ...
  console.log(`[GPT-Nexus] Simulating click on top node (Attempt ${state.attempt + 1})...`);
  ...
  try {
    ...
    console.error('[GPT-Nexus] Failed to click top node:', e);
  }
  ...
  if (changed) {
    console.log('[GPT-Nexus] History expanded (soft refresh). Clicking top node again...');
  ...
  console.log('[GPT-Nexus] No refresh or update detected. Exporting...');
  sessionStorage.removeItem(SESSION_KEY_PENDING_EXPORT);
  await performFinalExport(
    format,
    dict,
...
async function performFinalExport(
  ...
  exportBtn.addEventListener('click', async (ev) => {
    ...
    try {
      ...
      console.error('[GPT-Nexus] Export error:', err);
...
async function checkPendingExport() {
  try {
    ...
    console.log('[GPT-Nexus] Resuming pending export sequence...');
    ...
    console.error('[GPT-Nexus] Failed to resume pending export:', e);
    sessionStorage.removeItem(SESSION_KEY_PENDING_EXPORT);
  }
}
...
async function handleResponseCopyImageClick(
  ...
  try {
    ...
    console.error('[GPT-Nexus] Failed to copy response image:', error);
...
export async function startExportButton(): Promise<void> {
    ...
    window.addEventListener(
      ...
      () => {
        try {
          ...
          console.error('[GPT-Nexus] Failed to remove storage listener on unload:', e);
        }
      },
      { once: true },
    );
  ...
  btn.addEventListener('click', (ev) => {
    ...
    try {
      ...
      try {
        console.error('GPT-Nexus export failed', err);
      } catch {}
    }
  });
  ...
  const reinjectExportButtonIfNeeded = () => {
    ...
    reinjectTimer = setTimeout(() => {
      ...
      try {
        ...
        try {
          console.debug('[GPT-Nexus] Export button re-injection failed:', e);
        } catch {}
      }
    }, 800);
  };
