...
const STYLE_ID = 'gpt-nexus-chat-width';
  ...
  chrome.storage?.sync?.get(['gptChatWidth', ENABLED_KEY], (res) => {
    ...
    if (typeof storedWidth === 'number' && storedWidth !== normalized) {
      try {
        ...
        console.warn('[GPT-Nexus] Failed to migrate chat width to %:', e);
      }
    }
  });
      ...
      if (typeof newWidth === 'number') {
        ...
        if (normalized !== newWidth) {
          try {
            ...
            console.warn('[GPT-Nexus] Failed to migrate chat width to % on change:', e);
          }
        }
      }
    }
