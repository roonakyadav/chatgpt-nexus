...
export async function startPromptManager(): Promise<{ destroy: () => void }> {
  ...
  try {
    ...
    // the header controls row prevents the "GPT-Nexus" title from being squeezed
        ...
        if (!browser.runtime?.id) {
          ...
          setNotice(
            i18n.t('pm_settings_fallback') ||
              'Open extension settings to configure Prompt Manager.',
            'err',
          );
          return;
        }
    ...
    backupBtn.addEventListener('click', async () => {
      try {
        ...
        const timelineHierarchyPayload = {
          format: 'gpt-nexus.timeline-hierarchy.v1' as const,
          exportedAt: new Date().toISOString(),
          version: EXTENSION_VERSION,
