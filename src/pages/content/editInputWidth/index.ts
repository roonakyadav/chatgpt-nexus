...
const STYLE_ID = 'gpt-nexus-edit-input-width';
  ...
  chrome.storage?.sync?.get(['gptEditInputWidth', ENABLED_KEY], (res) => {
    ...
    if (typeof storedWidth === 'number' && storedWidth !== normalized) {
      try {
        ...
        console.warn('[GPT-Nexus] Failed to migrate edit input width to %:', e);
      }
    }
  });
      ...
      if (typeof newWidth === 'number') {
        ...
        if (normalized !== newWidth) {
          try {
            ...
            console.warn('[GPT-Nexus] Failed to migrate edit input width to % on change:', e);
          }
        }
      }
    }
