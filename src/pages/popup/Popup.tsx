...
export default function Popup() {
  ...
  const [previewingEffect, setPreviewingEffect] = useState<VisualEffect | null>(null);
  ...
  const handleEffectHover = useCallback((effect: VisualEffect) => {
    if (effect === visualEffect) return;
    ...
    previewTimeoutRef.current = setTimeout(() => {
      ...
      void setSyncStorage({ [StorageKeys.GV_VISUAL_EFFECT]: effect });
      void browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
        if (tabs[0]?.id) {
          void browser.tabs.sendMessage(tabs[0].id, { type: 'INITIALIZE_VISUAL_EFFECTS' }).catch(() => {});
        }
  ...
  }, [visualEffect, setSyncStorage]);

  const handleEffectLeave = useCallback(() => {
    if (previewTimeoutRef.current) {
      clearTimeout(previewTimeoutRef.current);
    ...
    if (previewingEffect) {
      ...
      void setSyncStorage({ [StorageKeys.GV_VISUAL_EFFECT]: visualEffect });
      void browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
        if (tabs[0]?.id) {
          void browser.tabs.sendMessage(tabs[0].id, { type: 'INITIALIZE_VISUAL_EFFECTS' }).catch(() => {});
        }
  ...
  }, [previewingEffect, visualEffect, setSyncStorage]);

  const handleEffectClick = useCallback(async (effect: VisualEffect) => {
    // Cancel any preview
    if (previewTimeoutRef.current) {
      clearTimeout(previewTimeoutRef.current);
      previewTimeoutRef.current = null;
                        ...
                        return (
                          <label
                            ...
                            className={`
                              ...
                              ${isSelected 
                                ...
                                : isPreviewing
                                ? 'border-blue-500/60 shadow-[0_0_20px_rgba(59,130,246,0.4)] bg-blue-500/5'
                                : 'border-border/50 hover:border-border hover:shadow-lg bg-card'
                              }
                            `}
                            htmlFor={`effect-${effectValue}`}
                            onMouseEnter={() => handleEffectHover(effectValue as VisualEffect)}
                          ...
                          >
                            <input
                              ...
                              value={effectValue}
                              checked={isSelected}
                              onChange={() => handleEffectClick(effectValue as VisualEffect)}
                              className="sr-only"
                            />
